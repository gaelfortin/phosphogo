...
## Using the user-friendly version of phosphogo
You can use the user-friendly interface of pĥosphogo with:
```{r eval=FALSE}
library(phosphogo)
library(purrr)
phosphogoApp()
...
for(i in kinases_enrichment_files){ 
  deregulated_kinases <- c(deregulated_kinases,
                            readr::read_csv(i) %>%
                               dplyr::filter(up_vs_down_FDR < 0.05) %>%
                               dplyr::pull(top_predicted_kinase))
                  }
